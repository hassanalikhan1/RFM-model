...
k = [5,7,10] #number of clusters
times=[]
for m in range(len(k)):
	start_time = time.time()

	# X coordinates of random centroids
	C_x = np.random.randint(0, np.max(X[0]), size=k[m])
	# Y coordinates of random centroids
	C_y = np.random.randint(0, np.max(X[1]), size=k[m])
	C_z = np.random.randint(0, np.max(X[2]), size=k[m])
